...
def stat_parav(all_unique_points,noc_triangles_coor,noc_triangles,all_triangles_coor,all_triangles,stat_sol):
    ...
    conn = np.zeros(3*all_triangles.shape[0])
    # Define offset of the last vertex of each element
    offset = np.zeros(all_triangles.shape[0])
    # Define cell types
    ctype = np.zeros(all_triangles.shape[0])
    
    for ind, triangle in enumerate(all_triangles):
        conn[3*ind], conn[3*ind + 1], conn[3*ind +2] = triangle[0], triangle[1], triangle[2]  # first triangle
        offset[ind] = 3*ind +3
        ...
        if faces_data:
            for face in faces_data:
                ...
                for point in face_pts:
                    ...
                    added_positions.append(pt_position)
                ...
                if (not face_rep in faces_rep) or block_ind == 1:
                    ...
                    blocks = np.append(blocks, np.array([[block_ind, len(faces_rep), block_name, face_ind  , 0,-1,'first']]), axis = 0)
                    for ind in added_positions:
                        faces = np.append(faces, np.array([[len(faces_rep), face_ind, ind +1 ]]),axis = 0) #ind or ind+1
                else:
                    contacts_ind += [faces_rep.index(face_rep) + 1 ]
                    blocks = np.append(blocks, np.array([[block_ind, faces_rep.index(face_rep) + 1, block_name, face_ind, 0 , -1,'second' ]]), axis = 0)
...
def problem_building_up(blocks_centroid,points,faces,blocks,local_ref,blocks_att):
    ...
    for block_ind, block_centroid in enumerate(blocks_centroid[:,-3:]):
        #block_centroid = manifold_data[block_ind]['centroid']
        start = time.time()
        block_faces = np.where( (blocks[:,0] == block_ind +1))
        faces_ind = [ blocks[element][1] for element in block_faces[0]]
        faces_ind_old = [ blocks[element][3] for element in block_faces[0]]
        ...
        for ind, face in  enumerate(faces_ind):
            ...
            start = time.time()
            mesh_points, triangle_cells = face_mesh(face_points_coor)
            print("meshing done.")
            end = time.time()
            ...
            mesh_points = np.array([[np.round(float(element),30) for element in row ] for row in mesh_points])
            
            if (blocks[block_faces[0][ind],4] != 0 ):
                
                
                #mesh_points = face_points_coor # no mesh effect

                rhos = np.array([1, 1, 1])
                print("face",faces_ind_old[ind])
                print(local_ref)
                local_ref_face = local_ref[ np.where(local_ref[:,0] == faces_ind_old[ind])[0] ][0]
                ...
                face_first_point = len(equilibrium_fx)
                
                equilibrium_fx += [  0 , 0, 0  ]* mesh_points.shape[0] 
                equilibrium_fy += [   0 , 0, 0]* mesh_points.shape[0]
                equilibrium_fz += [  0,0, 0]* mesh_points.shape[0]

                m_fx +=  [  0,0, 0]* mesh_points.shape[0]
                m_fy += [  0,0, 0]* mesh_points.shape[0]
                m_fz +=  [  0,0, 0]* mesh_points.shape[0]
            ...
            print("first part", time.time() - start_w)
            ...
            for triangle in triangle_cells:
                ...
                triangle_coordinates = mesh_points[triangle]
                #fig = blocks_plot(triangle_coordinates,fig)
                ...
                if ( (blocks[block_faces[0][ind],4] != 0)  ):
                   # Example
                    ...
                    if (not (face in faces_ind_read)) or (all(blocks[block_faces[0][ind],4] != num for num in [0, 1])) :   
                        all_triangles = np.append(all_triangles , np.array([[all_unique_points.shape[0] + triangle[0], all_unique_points.shape[0] + triangle[1], all_unique_points.shape[0] + triangle[2] ]]),axis = 0) 
                        all_triangles_coor.append(np.array(triangle_coordinates))
                    
                    end = time.time()
                    part2 += end - start

                    start  =time.time()
                    coef, coef_m = integral_gauss(triangle_coordinates,rhos,True, normal, x_local, y_local, block_centroid)
                    ...
                    for index, node in enumerate(triangle):
                        equilibrium_fx[face_first_point + 3*node] += coef[index]*x_local[0]
                        equilibrium_fx[face_first_point + 3*node +1] += coef[index]*y_local[0]
                        equilibrium_fx[face_first_point + 3*node +2] += coef[index]*normal[0]

                        equilibrium_fy[face_first_point + 3*node] += coef[index]*x_local[1]
                        equilibrium_fy[face_first_point + 3*node +1] += coef[index]*y_local[1]
                        equilibrium_fy[face_first_point + 3*node +2] += coef[index]*normal[1]

                        equilibrium_fz[face_first_point + 3*node] += coef[index]*x_local[2]
                        equilibrium_fz[face_first_point + 3*node +1] += coef[index]*y_local[2]
                        equilibrium_fz[face_first_point + 3*node +2] += coef[index]*normal[2]

                
                        point = triangle_coordinates[index]
                        ...
                        m_fx[face_first_point + 3*node] += coef_m[index][0]
                        m_fx[face_first_point + 3*node +1] += coef_m[index][1]
                        m_fx[face_first_point + 3*node +2] += coef_m[index][2]
                   
                        
                        m_fy[face_first_point + 3*node] += coef_m[index][3]
                        m_fy[face_first_point + 3*node +1] += coef_m[index][4]
                        m_fy[face_first_point + 3*node +2] += coef_m[index][5]

                        m_fz[face_first_point + 3*node] += coef_m[index][6]
                        m_fz[face_first_point + 3*node +1] += coef_m[index][7]
                        m_fz[face_first_point + 3*node +2] += coef_m[index][8]
                    end = time.time()
                    mat_time += end - start
                else:
                    face_first_point = len(equilibrium_fx)
                    noc_triangles = np.append(noc_triangles , np.array([[face_first_point + triangle[0], face_first_point + triangle[1], face_first_point + triangle[2] ]]),axis = 0) 
                    noc_triangles_coor.append(np.array(triangle_coordinates))
            ...
            if  (blocks[block_faces[0][ind],4] != 0) :
                part_to_add = np.array([[str(block_ind +1), str(faces_ind[ind]), str(faces_ind_old[ind])] for _ in range(mesh_points.shape[0])])
                first_ind = all_mesh_points.shape[0]
                mesh_points = np.append(mesh_points, part_to_add,axis = 1)
                
                all_mesh_points = np.append( all_mesh_points, mesh_points,axis = 0)
                
                if float(blocks[block_faces[0][ind],5]) != -1:
                    all_coh_ind.append(list(range(first_ind , first_ind+ part_to_add.shape[0]))) 
                    all_coh_val.append(float(blocks[block_faces[0][ind],5]))


                if (not (face in faces_ind_read)) or (all(blocks[block_faces[0][ind],4] != num for num in [0, 1])):  
                    if (blocks[block_faces[0][ind],4] == 3):
                        all_supports_ind.extend(range(first_ind , first_ind+ part_to_add.shape[0]))
                    
                    all_unique_points = np.append( all_unique_points, mesh_points,axis = 0)
        ...
        del equilibrium_block, equilibrium_fx, equilibrium_fy,equilibrium_fz, m_fx, m_fy, m_fz
        ...
        equilibrium_matrix.extend( nonzero_elements)
        ...
        #contact_pos = np.where( (blocks[:,0] == str(block_ind +1)) & ( (blocks[:,4] == str(1) ) | (blocks[:,4] == str(2) )  ) )
    ...
    #equilibrium_matrix_col.insert(0,3*n)
    #MCoul_matrix_sparse = spmatrix(MCoul_elements, MCoul_row_indices, MCoul_col_indices, (4*n, 3*n +1 ))
    
    
    
    
    equilibrium_matrix_sparse = spmatrix(equilibrium_matrix, equilibrium_matrix_row, equilibrium_matrix_col, (6*nb, 3*n + 1))            
    ...
    for block_ind, block_volume in enumerate(blocks_volume):
        b[6*block_ind +2] = density*block_volume[1] -3*0
    ...
    stat_parav(all_unique_points,noc_triangles_coor,noc_triangles,all_triangles_coor,all_triangles,xx[0:3*nef+1])
    print("ubbb",ub)
    print(all_coh_ind)
    print(all_coh_val)
    return ub
